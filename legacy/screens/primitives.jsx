...
// Leaf mark — the brand logo
function LeafMark({ size = 22, color = '#225A40' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
      ...
      <path d="M16 6 L 16 26 M 16 14 L 22 12 M 16 14 L 10 12 M 16 20 L 23 19 M 16 20 L 9 19"
        stroke="#F7F1E3" strokeWidth="0.9" strokeLinecap="round" opacity="0.7"/>
    </svg>
  );
}

// Small icon set — line style, culturally resonant
const Icons = {
...
// Produce emoji stand-ins drawn with CSS (culturally specific)
function ProduceGlyph({ kind, size = 56 }) {
  const styles = {
  ...
  const s = styles[kind] || styles.aguacate;
  return (
    <div style={{
...
function ProduceShape({ kind, fg, size }) {
  if (kind === 'pear') return (
...
Object.assign(window, { LeafMark, Icons, ProduceGlyph });
