...
// Safari trackpad pinch fires non-standard `gesture*` events. Type them locally
// so we don't need lib.dom updates and stay strict-mode clean.
...
export function DesignCanvas({
  children, minScale = 0.1, maxScale = 8, style = {},
}: {
  children: ReactNode;
  minScale?: number;
  maxScale?: number;
  ...
  useEffect(() => {
    ...
    // A physical wheel sends line-mode deltas (Firefox) or large integer pixel
    // deltas with no X component (Chrome/Safari). Trackpad scroll sends small
    // fractional deltas, often with non-zero deltaX. ctrlKey means trackpad pinch.
...
export function DCSection({
  title, subtitle, children, gap = 48,
}: { title: ReactNode; subtitle?: ReactNode; children: ReactNode; gap?: number }) {
  return (
...
export function DCArtboard({
  label, children, width, height, style = {},
}: {
  label?: ReactNode;
  children: ReactNode;
  width?: number;
...
export function DCPostIt({
  children, top, left, right, bottom, rotate = -2, width = 180,
}: {
  children: ReactNode;
  top?: number;
  left?: number;
